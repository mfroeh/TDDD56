...
int initOpenCL() {
  ...
  ciErrNum = clGetDeviceInfo(device, CL_DEVICE_MAX_WORK_GROUP_SIZE,
                             sizeof(size_t), &noWG, NULL);
  ...
  //sizeof(workitem_size), &workitem_size, NULL); 	printf("
  //CL_DEVICE_MAX_WORK_ITEM_SIZES:\t%u / %u / %u \n", workitem_size[0],
  //workitem_size[1], workitem_size[2]);

  // create command queue
  commandQueue =
      clCreateCommandQueueWithProperties(cxGPUContext, device, 0, &ciErrNum);
