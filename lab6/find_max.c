// Laboration in OpenCL. Based on a lab by Jens Ogniewski and Ingemar Ragnemalm
// 2010-2011. Rewritten by Ingemar 2017. Very close to the shell for bitonic
// sort.

// Compilation line for Linux:
// test$ gcc -std=c99 find_max.c -o find_max milli.c CLutilities.c -lOpenCL
...
int find_max_gpu(unsigned int *data, unsigned int length)
{
  ...
  io_data =
      clCreateBuffer(cxGPUContext, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR,
  ...
  cl_event event;
  ciErrNum =
      clEnqueueReadBuffer(commandQueue, io_data, CL_TRUE, 0,
                          length * sizeof(unsigned int), data, 0, NULL, &event);
...
int main(int argc, char **argv)
{
  ...
  // ResetMilli(); // You may consider moving this inside find_max_gpu(), to
  // skip timing of data allocation.
