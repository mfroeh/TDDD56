...
#ifdef MEASURE
#include <time.h>
// If we measure, we don't debug as assert() and printf() seriously affect
// performance
#undef DEBUG
#endif
...
static void compute_chunk(struct mandelbrot_param *args) {
  ...
  for (i = args->begin_h; i < args->end_h; i++) {
    ...
    for (j = args->begin_w; j < args->end_w; j++) {
      ...
      pixel = val > args->maxiter ? args->mandelbrot_color
                                  : color[val % num_colors(args)];
...
void init_round(struct mandelbrot_thread *args) {
  // Initialize or reinitialize here variables before any thread starts or
  // restarts computation Every thread run this function; feel free to allow
  // only one of them to do anything
}

/*
 * Each thread starts individually this function, where args->id give the
 * thread's id from 0 to NB_THREADS
 */
void parallel_mandelbrot(struct mandelbrot_thread *args,
                         struct mandelbrot_param *parameters) {
...
#if NB_THREADS > 0
static void *run_thread(void *buffer) {
...
void init_ppm(struct mandelbrot_param *param) {
  ...
  param->picture->data = malloc(
      ppm_align(sizeof(color_t) * param->width, PPM_ALIGNMENT) * param->height);
...
void update_colors(struct mandelbrot_param *param) {
  ...
  for (i = 0; i < num_colors(param); i++) {
    color[i].green =
        (stop.green - start.green) * ((double)i / num_colors(param)) +
        start.green;
    color[i].red =
        (stop.red - start.red) * ((double)i / num_colors(param)) + start.red;
    color[i].blue =
        (stop.blue - start.blue) * ((double)i / num_colors(param)) + start.blue;
  }
}
...
void init_mandelbrot(struct mandelbrot_param *param) {
  ...
  for (int i = 0; i < param->height; i++){
    ...
    descriptor.fun_ptr = compute_chunk;
    descriptor.begin_h = i;
    descriptor.end_h = i+1;
    
    task_pool[i] = descriptor;
  }
#endif
#if GLUT != 1
  ...
  for (i = 0; i < NB_THREADS; i++) {
    ...
    // Check the good behavior or pthread_create; must be disabled while
    // measuring for performance reasons
#ifdef DEBUG
    assert(pthread_create(&thread[i], &thread_attr, &run_thread,
                          &thread_data[i]) == 0);
#else
