// Mandelbrot explorer, based on my old Julia demo plus parts of Nicolas Melot's
// Lab 1 code. CPU only! Your task: Rewrite for CUDA! Test and evaluate
// performance.

// Compile with:
// gcc interactiveMandelbrot.cpp -shared-libgcc -lstdc++-static  -o
// interactiveMandelbrot -lglut -lGL or g++ interactiveMandelbrot.cpp -o
// interactiveMandelbrot -lglut -lGL
...
__global__ void mandelbrot(int* iters, int gImageWidth, int gImageHeight,
  ...
  MYFLOAT jx{scale *
             (static_cast<MYFLOAT>(gImageWidth) / 2 - x + offsetx / scale) /
             (static_cast<MYFLOAT>(gImageWidth) / 2)};
  MYFLOAT jy{scale *
             (static_cast<MYFLOAT>(gImageHeight) / 2 - y + offsety / scale) /
             (static_cast<MYFLOAT>(gImageHeight) / 2)};
...
void computeFractal(unsigned char* ptr) {
  ...
  for (int y = 0; y < gImageHeight; y++) {
    for (int x = 0; x < gImageWidth; x++) {
      ...
      int fractalValue = iters[y * gImageWidth + x];
...
static void print_str(void* font, const char* string) {
  ...
  for (i = 0; string[i]; i++) glutBitmapCharacter(font, string[i]);
}
...
void Draw() {
  ...
  // Dump the whole picture onto the screen. (Old-style OpenGL but without lots
  // of geometry that doesn't matter so much.)
  glClearColor(0.0, 0.0, 0.0, 1.0);
  glClear(GL_COLOR_BUFFER_BIT);
  glDrawPixels(gImageWidth, gImageHeight, GL_RGBA, GL_UNSIGNED_BYTE, pixels);

  if (print_help) PrintHelp();
