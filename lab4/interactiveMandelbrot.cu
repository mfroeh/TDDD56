...
__global__ void mandelbrot(int* iters, int gImageWidth, int gImageHeight, MYFLOAT offsetx, MYFLOAT offsety, MYFLOAT scale, int maxiter)
{
	unsigned x{blockIdx.x * blockDim.x + threadIdx.x};
	unsigned y{blockIdx.y * blockDim.y + threadIdx.y};

    MYFLOAT jx = scale * (MYFLOAT)(gImageWidth/2 - x + offsetx/scale)/(gImageWidth/2);
    MYFLOAT jy = scale * (MYFLOAT)(gImageHeight/2 - y + offsety/scale)/(gImageHeight/2);
...
void computeFractal( unsigned char *ptr)
{
	...
	dim3 block_dim{1, 1};
	dim3 grid_dim(gImageWidth, gImageHeight);
	mandelbrot<<<grid_dim, block_dim>>>(iters_dev, gImageWidth, gImageHeight, offsetx, offsety, scale, maxiter);
	cudaDeviceSynchronize();

	int* iters = new int[sizeof(int) * gImageWidth * gImageHeight];
	cudaMemcpy(iters, iters_dev, sizeof(int) * gImageWidth * gImageHeight, cudaMemcpyDeviceToHost);
